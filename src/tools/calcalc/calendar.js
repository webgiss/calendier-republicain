...
import { amod, deltat, equationOfTime, equinox, jhms, jwday, mod, TropicalYear, Weekdays } from "./astro";
...
export function nearest_weekday(weekday, jd) {
...
export function next_or_current_weekday(weekday, jd) {
...
export function previous_or_current_weekday(weekday, jd) {
...
export const JULIAN_EPOCH = 1721423.5;
...
export const HEBREW_EPOCH = 347995.5;
...
export function jd_to_french_revolutionary(jd) {
...
export function french_revolutionary_to_jd(an, mois, decade, jour) {
...
export function updateFromGregorian(document) {
    var j, year, mon, mday, hour, min, sec,
        weekday, julcal, hebcal, islcal, hmindex, utime, isoweek,
        may_countcal, mayhaabcal, maytzolkincal, frrcal,
        indcal, isoday;

    year = document.gregorian.year.value;
    mon = document.gregorian.month.selectedIndex;
    mday = document.gregorian.day.value;
    hour = min = sec = 0;
    hour = document.gregorian.hour.value;
    min = document.gregorian.min.value;
    sec = document.gregorian.sec.value;
    ...
    switch (hebrew_year_days(hebcal[0])) {
        ...
        default:
            document.hebrew.leap.value = `Invalid year length: ${hebrew_year_days(hebcal[0])} days.`;
            break;
    }
...
export function calcGregorian(document) {
    updateFromGregorian(document);
}

//  calcJulian  --  Perform calculation starting with a Julian date

export function calcJulian(document) {
    var j, date, time;

    j = document.julianday.day.value;
...
export function setJulian(document, j) {
    document.julianday.day.value = j;
    calcJulian(document);
}
...
export function calcModifiedJulian(document) {
    setJulian(document, (document.modifiedjulianday.day.value) + JMJD);
}

//  calcJulianCalendar  --  Update from Julian calendar

export function calcJulianCalendar(document) {
    setJulian(
        document,
        julian_to_jd(
            document,
            (document.juliancalendar.year.value),
            document.juliancalendar.month.selectedIndex + 1,
            (document.juliancalendar.day.value)
        )
    );
}
...
export function calcHebrew(document) {
    setJulian(
        document,
        hebrew_to_jd(
            (document.hebrew.year.value),
            document.hebrew.month.selectedIndex + 1,
            (document.hebrew.day.value)
        )
    );
}
...
export function calcIslamic(document) {
    setJulian(
        document,
        islamic_to_jd(
            (document.islamic.year.value),
            document.islamic.month.selectedIndex + 1,
            (document.islamic.day.value)
        )
    );
}
...
export function calcPersian(document) {
    setJulian(
        document,
        persian_to_jd(
            (document.persian.year.value),
            document.persian.month.selectedIndex + 1,
            (document.persian.day.value)
        )
    );
}
...
export function calcPersiana(document) {
    setJulian(
        document,
        persiana_to_jd(
            (document.persiana.year.value),
            document.persiana.month.selectedIndex + 1,
            (document.persiana.day.value)
        ) + 0.5
    );
}
...
export function calcMayanCount(document) {
    setJulian(document,
        mayan_count_to_jd(
            (document.mayancount.baktun.value),
            (document.mayancount.katun.value),
            (document.mayancount.tun.value),
            (document.mayancount.uinal.value),
            (document.mayancount.kin.value)
        )
    );
}
...
export function calcIndianCivilCalendar(document) {
    setJulian(
        document,
        indian_civil_to_jd(
            (document.indiancivilcalendar.year.value),
...
export function calcFrench(document) {
    let j = document.french.jour.selectedIndex;
    let decade = document.french.decade.selectedIndex;
    let mois = document.french.mois.selectedIndex;
    ...
    setJulian(french_revolutionary_to_jd((document.french.an.value),
        mois + 1,
        decade + 1,
        j + 1));
}
...
export function calcGregSerial(document) {
    setJulian(
        document,
        (document.gregserial.day.value) + J0000
    );
}

//  calcExcelSerial1900  --  Perform calculation starting with an Excel 1900 serial date

function calcExcelSerial1900(document) {
    var d = document.excelserial1900.day.value;
    ...
    setJulian(document, (d - 1) + J1900);
}

//  calcExcelSerial1904  --  Perform calculation starting with an Excel 1904 serial date

function calcExcelSerial1904(document) {
    setJulian(document, (document.excelserial1904.day.value) + J1904);
}

//  calcUnixTime  --  Update from specified Unix time() value

function calcUnixTime(document) {
    var t = document.unixtime.time.value;

    setJulian(document, J1970 + (t / (60 * 60 * 24)));
}

//  calcIsoWeek  --  Update from specified ISO year, week, and day

function calcIsoWeek(document) {
    var year = document.isoweek.year.value,
        week = document.isoweek.week.value,
        day = document.isoweek.day.value;

    setJulian(document, iso_to_julian(year, week, day));
}

//  calcIsoDay  --  Update from specified ISO year and day of year

function calcIsoDay(document) {
    var year = document.isoday.year.value,
        day = document.isoday.day.value;

    setJulian(document, iso_day_to_julian(year, day));
}
...
function setDateToToday(document) {
    var today = new Date();
...
export function presetDataToRequest(document, s) {
    var eq = s.indexOf("=");
    var set = false;
    if (eq !== -1) {
        ...
        if (calendar.toLowerCase() === "gregorian") {
            ...
            if (d !== null) {
                ...
                if ((d[2] >= 1) && (d[2] <= 12) &&
                    ...
                    alert(`Invalid Julian calendar date "${date}" in search request`);
                }
            } else {
                alert(`Invalid Julian calendar date "${date}" in search request`);
            }
            ...
            let d = date.match(/^(-?\d+\.?\d*)/);
            if (d !== null) {
                setJulian(document, d[1]);
                set = 1;
            } else {
                alert(`Invalid Julian day "${date}" in search request`);
            }
            ...
            if (d !== null) {
                ...
                calcModifiedJulian(document);
                set = 1;
            } else {
                alert(`Invalid Modified Julian day "${date}" in search request`);
            }
            ...
            if ((d = date.match(/^(-?\d+)-(\d\d\d)/)) !== null) {
                ...
                calcIsoWeek(document);
                set = 1;
            } else {
                alert(`Invalid ISO-8601 date "${date}" in search request`);
            }
            ...
            if (d !== null) {
                ...
                calcExcelSerial1900(document);
                set = 1;
            } else {
                alert(`Invalid Excel serial day (1900/PC) "${date}" in search request`);
            }
            ...
            if (d !== null) {
                ...
                calcExcelSerial1904(document);
                set = 1;
            } else {
                alert(`Invalid Excel serial day (1904/Mac) "${date}" in search request`);
            }

        } else {
            alert(`Invalid calendar "${calendar}" in search request`);
        }
    } else {
    ...
    if (!set) {
        setDateToToday(document);
        calcGregorian(document);
    }
}
